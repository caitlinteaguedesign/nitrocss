...
{	
	$(".read-more__content").each(function(index) {	// Each div with text that might need to be shortened
		...
		if( height > px) {							// Too tall - shorten it
...
$(document).on("click", ".read-more__button", function(){
	...
	if( $("#fade-"+ID).css('display') == 'block') { 	// Text is already expanded - close it
...
$(document).click(function(e) {
	if (e.target.id != '.toggle-tooltip' && !$('.toggle-tooltip').find(e.target).length) {
...
function setScrollableShadowWidth($scrollable) {
	...
	$scrollable.find(".scrollable__shadow").css("width", "calc(100% - "+width+")"); 
}
...
function dynamicScrollableShadows($container) {
	...
	var bottom = $container.parent(".scrollable").find(".scrollable__shadow-bottom");
